        ...
        try {
            ...
            if (ws.mouseDown()) {
                ...
                if (voc > 0) {  // zero is for drum voices
                    ...
                    p = w.getPatchAt(x, y);
                    if (!MusicExtension.patchesMusicMap.containsKey(p)) { //p.pcolor().equals(P.BLACK)) {  // only paint empty patch
                        ...
                        for (int iy = (voc + 1) * P.PATCHESPERVOICE; iy < (voc + 2) * P.PATCHESPERVOICE; iy++) {
                            ...
                            MusicExtension.patchesMusicMap.remove(tmp);
                        }
                        // color this patch
                        p.setPatchVariable(pcoloridx, P.vcolor[voc]);
                        Note newnote = new Note (P.voices[voc].note( (int) y % P.PATCHESPERVOICE),
                                P.vcolor[voc], (int) y % P.PATCHESPERVOICE) ;
                        MusicExtension.patchesMusicMap.put(p,newnote);
                        Play.playnote(voc,newnote);
