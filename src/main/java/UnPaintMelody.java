        ...
        try {
            ...
            if (ws.mouseDown()) {
                ...
                if (voc >= 0 && voc < P.vcolor.length) {  // zero is for rhythm
                    p = w.getPatchAt(x , y);
                    if (org.nlogo.extensions.music.MusicExtension.patchesMusicMap.containsKey(p)) {//!p.pcolor().equals(P.BLACK)) { // unset sound
                        p.setPatchVariable(pcoloridx, P.DBLACK);
                        org.nlogo.extensions.music.MusicExtension.patchesMusicMap.remove(p);
