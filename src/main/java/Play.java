...
public class Play implements Command {
    ...
    */
    public void perform(Argument args[], Context context)
        ...
        try {
            ...
            for (int i = 0; i < P.NDRUMS; i++) {
                ...
                if (MusicExtension.patchesMusicMap.containsKey(p) ) { //!p.pcolor().equals(P.DBLACK)) {
                    playdrumnote(voc,p);//(double) (p.pcolor()));
    ...
    // MusicExtension.playNote is in terms of msec,
    ...
    public static void playnote(int vid, Note note) //int noteidx)
        ...
        if (note != null) {
            if (v.isMidi())
                MusicExtension.playNote(v.instrument,
                        note.getPitch(),
                        note.vel(v), // convert voice's velocity for this note
                        v.dur * P.MIN_NOTE_DUR);
            else
                MusicExtension.playWav(v, note.getScalepos(),
                        v.dur);
        }
    }


    public static void playdrumnote(Voice voc, Patch p) //double pcolor)
            throws ExtensionException {
        double pcolor = (double) (p.pcolor());
        // brightness encodes velocity
                Note note = MusicExtension.patchesMusicMap.get(p);
            if (voc.isMidi()) {

                //int vel = (int) ((127 * (((int)p.pcolor()) % 10)) / 9.0);
                MusicExtension.playDrum(voc.instrument, note.getVol());//vel);
            }
            else {
                int vel = (int)pcolor % 10;
                MusicExtension.playDrumWav(voc, note.getVol());//vel);
            }
        }
    ...
    public static Note getNote(World w, int vid) throws AgentException {
        ...
        for (int i = 0; i < P.PATCHESPERVOICE; i++) {
            ...
            if ( MusicExtension.patchesMusicMap.containsKey(p)) { //!p.pcolor().equals(P.DBLACK)) {
                return MusicExtension.patchesMusicMap.get(p);//new Note(P.voices[vid].note(i),(Double) p.pcolor(),i);
            }
        }
