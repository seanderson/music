...
public class Rhythm implements Command {
    ...
    public void perform(Argument args[], Context context)
        ...
        // Validate input
        if (voice < 0 || voice >= org.nlogo.extensions.music.P.NDRUMS)
        ...
        if (startmeas < 0 || startmeas >= org.nlogo.extensions.music.P.NMEASURES)
            throw new org.nlogo.api.ExtensionException("Invalid start measure: " + startmeas);
        if (nummeas < 0 || startmeas + nummeas > org.nlogo.extensions.music.P.NMEASURES)
        ...
        try {
            ...
            for (int x = startmeas * P.MAXNOTESPERMEASURE, pos = 0; x < (startmeas + nummeas) * P.MAXNOTESPERMEASURE;
                ...
                if (pttn.charAt(pos) == P.SIL) {
                    ...
                    MusicExtension.patchesMusicMap.remove(p);
                }
                else if (Character.isDigit(pttn.charAt(pos))) {
                    double vol = P.dcolor[voice] + Character.getNumericValue(pttn.charAt(pos));
                    p.setPatchVariable(pcoloridx,
                            new Double(vol)); //P.dcolor[voice] + Character.getNumericValue(pttn.charAt(pos))));
                    MusicExtension.patchesMusicMap.put(p,new Note(0,vol,0));
